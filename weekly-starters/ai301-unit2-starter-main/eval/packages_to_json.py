...
SECTION_STARTS = {
    ...
    "## Issue": "issue",
    "## Candidate claim comment": "claim_comment",
    "## Candidate repro report": "repro_report",
}
...
def split_sections(lines):
    ...
    sections = {"meta": [], "repo_facts": [], "issue": [],
                "highlights": [], "claim_comment": [], "repro_report": []}
    current = "meta"
    fenced = False
    for line in lines:
        if line.startswith("```"):
        ...
        if not fenced:
            ...
            if m:
                ...
                current = "highlights"
                continue
            hit = next((k for k in SECTION_STARTS if line.startswith(k)),
                       None)
            if hit:
...
def parse_bundle(path):
    ...
    out["claim_comment_markdown"] = \
        "\n".join(sections["claim_comment"]).strip()
    out["repro_report_markdown"] = \
        "\n".join(sections["repro_report"]).strip()
    return out
